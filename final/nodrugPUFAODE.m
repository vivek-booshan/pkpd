function dydt = nodrugODE(t, y, p)
    ...
    % Update dROS to include the impact of PUFA
    dROS = (...
        + p.basalROS * ROS * p.foodProductionMultiplier ...
        - (p.antioxidant+p.PUFA.effectOnROS) * ROS * p.foodAntiOxMultiplier ...
        );

% Update peripheral cholesterol to include PUFA's effects on LDL and HDL
    dperipheral_chol = (...
    + p.V.GI * p.GI.chylomicron.chol * GI_chol ...
    + p.V.liver * p.liver.LDL.chol * liver_chol * ROS ...
    + p.V.liver * p.liver.HDL.chol * liver_chol ...
    - p.V.peripheral * p.peripheral.LDL.chol * peripheral_chol * (1 + p.PUFA.effectOnLDL) ...
    - p.V.peripheral * p.peripheral.HDL.chol * peripheral_chol * (1 - p.PUFA.effectOnHDL) ...
